...
export async function processSegmentationVolume(outLabelVolume, niftiImage, modelEntry, opts) {
  ...
  if (opts.isPostProcessEnable) {
    ...
    // Binary brain extraction/masking always keeps one connected foreground
    // component, regardless of its menu id.
    if (modelEntry.type === 'Brain_Extraction' || modelEntry.type === 'Brain_Masking') {
      binarize = true;
      onlyLargest = true;
    } else if ([1, 7].includes(modelEntry.id)) {
