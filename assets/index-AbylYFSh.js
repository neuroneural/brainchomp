...
`}if(P.length<1&&window.webgpuDiagnostics,P.length<1){wa("Diagnostics","No diagnostic string generated: run a model to create diagnostics");return}let r=P;l=l.slice(0,-2),l!==""&&r.includes("Status: OK")&&(r=r.replace("Status: OK",`Status: ${l}`)),l="",navigator.clipboard.writeText(r).then(()=>{wa("Diagnostics",`<p>Diagnostics copied to clipboard</p><pre style="white-space: pre-wrap; font-family: monospace; font-size: 0.9em; overflow-x: auto;">${r}</pre>`)}).catch(k=>{wa("Diagnostics",`<p>Failed to copy to clipboard.</p><pre style="white-space: pre-wrap; font-family: monospace; font-size: 0.9em; overflow-x: auto;">${r}</pre>`)})},opacitySlider0.oninput=function(){o.setOpacity(0,opacitySlider0.value/255),o.updateGLVolume()},opacitySlider1.oninput=function(){o.setOpacity(1,opacitySlider1.value/255)};function g(){return N(this,null,function*(){const P=o.volumes[0],r=256*256*256;if(!(P.dims[1]===256&&P.dims[2]===256&&P.dims[3]===256&&P.img.length===r))throw new Error(`Brainchomp currently requires a pre-conformed 256 × 256 × 256 NIfTI; received ${P.dims[1]} × ${P.dims[2]} × ${P.dims[3]}.`)})}function f(){return N(this,null,function*(){for(;o.volumes.length>1;)yield o.removeVolume(o.volumes[1])})}function M(){return o.volumes.length>=2&&o.volumes[1].colormapLabel?o.volumes[1]:null}function G(){b=null,s=null,c=null}function D(){const P=M();if(P){if(s===null&&(s=P.img),b===null)P.img=s,c=null;else{const r=s,k=new r.constructor(r.length);for(let q=0;q<r.length;q++)k[q]=r[q]===b?b:0;P.img=k,c=I(b)}o.updateGLVolume()}}function I(P){const r=o.volumes[0],k=s||o.volumes[1]&&o.volumes[1].img;if(!r||!k)return null;const q=r.hdr.pixDims||[],K=q[1]&&q[2]&&q[3]?q[1]*q[2]*q[3]:1,A=oa(r.img,k,K),na=A.reduce((la,ca)=>la+ca.volume_mm3,0),Q=A.find(la=>la.label===P);if(!Q)return null;const j=na>0?Q.volume_mm3/na*100:0,W=[Q.name,`${Ma(Q.volume_mm3)} cm3   (${j.toFixed(1)}% of brain)`,`${Q.voxels.toLocaleString()} voxels`,`intensity  ${Q.mean.toFixed(0)} +/- ${Q.stdev.toFixed(0)}`];let Y=[1,1,1,1];if(n&&n.R&&n.R[P]!=null){const la=ca=>Math.min(255,ca*.55+130)/255;Y=[la(n.R[P]),la(n.G[P]),la(n.B[P]),1]}return{lines:W,color:Y}}function z(){if(b===null||!c||!M())return;const P=o.screenSlices&&o.screenSlices.find(ca=>ca.axCorSag===4);if(!P)return;const[r,k]=P.leftTopWidthHeight,q=o.gl;q.viewport(0,0,q.canvas.width,q.canvas.height),q.enable(q.BLEND);const A=o.fontPx*v*1.55,na=o.fontPx*.6,Q=r+na,j=k+na,W=[.92,.92,.92,1],{lines:Y,color:la}=c;o.drawText([Q,j],Y[0],v,la);for(let ca=1;ca<Y.length;ca++)o.drawText([Q,j+ca*A],Y[ca],v,W)}function R(){const P=M();if(!P)return null;const r=o.frac2mm(o.scene.crosshairPos,0,!0),k=P.mm2vox(r),q=P.img;s&&(P.img=s);const K=Math.round(P.getValue(k[0],k[1],k[2],P.frame4D));return P.img=q,K}function Z(P){M()&&(b=P===0||P===b?null:P,D())}function ia(P){if(!P[t]||!M())return;const r=R();r===null||Number.isNaN(r)||(Z(r),P.preventDefault())}function H(){return N(this,null,function*(){if(!Da){Da=!0,d&&(d.disabled=!0),Za=[];try{yield Ua()}catch(P){console.error("Inference could not start:",P),wa("Input not supported",qa((P==null?void 0:P.message)||String(P)))}finally{Da=!1,d&&(d.disabled=!1)}}})}function Ua(){return N(this,null,function*(){const P=modelSelect.value;if(P==="-1"||modelSelect.selectedIndex<0)return;yield f(),G(),yield g();const r=Ca[P],k=Ea({},ft),q=new URL("./",window.location.href).href;k.rootURL=q.endsWith("/")?q.slice(0,-1):q;const K=o.volumes[0].img;if($a&&!Ne&&!r.preferWebGL2&&r.webgpu_safetensor){console.log("Attempting WebGPU backend...");const j=Ba(Ea({},r),{enableTTA:!1});try{yield Wt(Ie,k,j,o.volumes[0].hdr,K,sa,va);return}catch(W){console.error("WebGPU inference failed, falling back to WebWorker.",W)}}if(r.webgpu_safetensor)try{const{runInferenceWebGl2:j,nativeWebgl2Available:W}=yield Ye(()=>N(null,null,function*(){const{runInferenceWebGl2:Y,nativeWebgl2Available:la}=yield import("./inference-webgl2-BmaBQJ4A.js");return{runInferenceWebGl2:Y,nativeWebgl2Available:la}}),[],import.meta.url);if(W()){console.log("Attempting native WebGL2 runner..."),yield j(k,r,o.volumes[0].hdr,K,sa,va);return}console.log("Native WebGL2 unavailable here (no OffscreenCanvas/webgl2); using the tfjs worker.")}catch(j){console.warn("Native WebGL2 declined or failed, falling back to the tfjs worker.",j.message)}if(console.log("Attempting WebWorker backend..."),typeof i!="undefined"){console.log("Worker is busy. Please wait.");return}const A={dims:o.volumes[0].hdr.dims,datatypeCode:o.volumes[0].hdr.datatypeCode},na=j=>new Promise((W,Y)=>{const la=Ba(Ea({},k),{enableSeqConv:j}),ca=Ba(Ea({},r),{enableSeqConv:j,enableTTA:!1});i=new Kt({}),i.postMessage({opts:la,modelEntry:ca,niftiHeader:A,niftiImage:K}),i.onmessage=function(ya){const{cmd:ma,message:Ra,progressFrac:Wa,modalMessage:fa,statData:La,img:Fe,opts:We,modelEntry:ze}=ya.data;if(ma==="ui"){if(fa){if(i.terminate(),i=void 0,La&&La.Status==="Fail"){Y(new Error(La.Error_Type||fa));return}if(typeof fa=="string"&&(fa.toLowerCase().includes("fail")||fa.toLowerCase().includes("error")||fa.toLowerCase().includes("compatible")||fa.toLowerCase().includes("texture")||fa.toLowerCase().includes("maximum"))){Y(new Error(fa));return}}va(Ra,Wa,fa,La)}ma==="img"&&(i.terminate(),i=void 0,sa(Fe,We,ze),W())},i.onerror=function(ya){console.error("WebWorker failed",ya),i.terminate(),i=void 0,Y(ya)}});try{console.log("Attempting WebWorker with enableSeqConv: false"),yield na(!1);return}catch(j){console.warn("WebWorker (fast) failed, retrying with enableSeqConv: true",j),typeof i!="undefined"&&(i.terminate(),i=void 0),console.log("Waiting 1000ms for WebGL context cleanup..."),yield new Promise(W=>setTimeout(W,1e3));try{console.log("Attempting WebWorker with enableSeqConv: true"),yield na(!0);return}catch(W){console.error("WebWorker (slow) failed, falling back to Main Thread.",W)}}console.log("Attempting Main Thread backend...");const Q=j=>new Promise((W,Y)=>{const la=Ba(Ea({},k),{enableSeqConv:j}),ca=Ba(Ea({},r),{enableSeqConv:j}),ya=(Ra,Wa,fa,La)=>{La&&La.Status==="Fail"?Y(new Error(La.Error_Type||fa||"Inference Failed")):fa&&typeof fa=="string"&&(fa.toLowerCase().includes("fail")||fa.toLowerCase().includes("error")||fa.toLowerCase().includes("compatible")||fa.toLowerCase().includes("texture")||fa.toLowerCase().includes("maximum"))&&Y(new Error(fa)),va(Ra,Wa,fa,La)},ma=(Ra,Wa,fa)=>{sa(Ra,Wa,fa),W()};Ot(la,ca,o.volumes[0].hdr,K,ma,ya).catch(Ra=>Y(Ra))});try{console.log("Attempting Main Thread with enableSeqConv: false"),yield Q(!1)}catch(j){console.warn("Main Thread (fast) failed, retrying with enableSeqConv: true",j),yield new Promise(W=>setTimeout(W,100));try{console.log("Attempting Main Thread with enableSeqConv: true"),yield Q(!0)}catch(W){console.error("Main Thread (slow) failed.",W),dl(W)}}})}modelSelect.onchange=H,d&&(d.onclick=()=>{modelSelect.value="0",H()}),y&&(y.onchange=()=>{S&&J().catch(P=>{console.error("Could not switch result overlay:",P),wa("Overlay error",qa((P==null?void 0:P.message)||String(P)))})});function Ta(P){const r=M(),k=b!==null&&r&&s;k&&(r.img=s);try{return P()}finally{k&&D()}}function _a(P){return N(this,null,function*(){if(!S||!_){window.alert("No result to save (run Skull-strip first).");return}const r=yield o.volumes[0].clone();if(r.zeroImage(),Object.assign(r.hdr,{scl_inter:0,scl_slope:1}),P==="mask"){r.img=S,r.hdr.datatypeCode=2,r.hdr.numBitsPerVoxel=8,r.hdr.intent_code=1002,r.saveToDisk("brainmask.nii.gz");return}r.img=_,r.hdr.intent_code=0,r.saveToDisk("skull_stripped_brain.nii.gz")})}function V(){if(o.volumes.length<1){window.alert("No image loaded.");return}o.volumes[0].saveToDisk("input.nii.gz")}function aa(){return N(this,null,function*(){if(o.volumes.length<1){window.alert("No image loaded.");return}yield Ta(()=>N(null,null,function*(){yield o.saveDocument("brainchomp.nvd")}))})}const ua=[{act:()=>_a("brain"),title:"Skull-stripped brain",sub:"input intensities with background set to zero",need:"result"},{act:()=>_a("mask"),title:"Brain mask",sub:"binary largest-component mask",need:"result"},{act:V,title:"Input volume",sub:"the currently loaded 256³ NIfTI",need:"img"},{act:aa,title:"Scene",sub:"everything, as a .nvd document",need:"img"}];function ea(){const P=o.volumes.length>=1,r=!!S,k=A=>A==="result"?r:P,q=ua.map((A,na)=>{const Q=k(A.need)?"":" disabled";return`<button type="button" class="save-opt${Q}" data-i="${na}"${Q?" disabled":""}>
        <span class="save-opt-title">${A.title}</span>
        <span class="save-opt-sub">${A.sub}</span>
      </button>`}).join("");wa("Save",`<div class="save-options">${q}</div>`,{hideClose:!0,saveMode:!0});const K=document.getElementById("dialogMessage");K&&K.querySelectorAll(".save-opt:not(.disabled)").forEach(A=>{A.onclick=()=>{const na=ua[parseInt(A.dataset.i,10)],Q=document.getElementById("appDialog");Q&&Q.open&&Q.close(),na.act()}})}const ta=document.getElementById("saveBtn");ta&&(ta.onclick=ea);function oa(P,r,k){const K=new Map,A=r.length;for(let j=0;j<A;j++){const W=r[j];if(W===0)continue;let Y=K.get(W);Y||(Y={count:0,sum:0,sumSq:0,hist:new Float64Array(256)},K.set(W,Y));const la=P[j];Y.count++,Y.sum+=la,Y.sumSq+=la*la,Y.hist[la]++}const na=(j,W,Y)=>{const la=Y*W;let ca=0;for(let ya=0;ya<j.length;ya++)if(ca+=j[ya],ca>=la)return ya;return j.length-1},Q=[];for(const[j,W]of[...K.entries()].sort((Y,la)=>Y[0]-la[0])){const Y=W.sum/W.count,la=Math.max(0,W.sumSq/W.count-Y*Y);let ca=0,ya=0;for(let ma=0;ma<W.hist.length;ma++)if(W.hist[ma]>0){ca=ma;break}for(let ma=W.hist.length-1;ma>=0;ma--)if(W.hist[ma]>0){ya=ma;break}Q.push({label:j,name:u&&u[j]!=null?u[j]:`label_${j}`,voxels:W.count,volume_mm3:W.count*k,min:ca,max:ya,q1:na(W.hist,W.count,.25),median:na(W.hist,W.count,.5),q3:na(W.hist,W.count,.75),mean:Y,stdev:Math.sqrt(la)})}return Q}function da(P){const r=["label","name","voxels","volume_mm3","min","max","q1","median","q3","mean","stdev"],k=A=>Number.isInteger(A)?String(A):A.toFixed(6),q=A=>/[",\n]/.test(A)?`"${String(A).replace(/"/g,'""')}"`:String(A),K=[r.join(",")];for(const A of P)K.push([A.label,q(A.name),A.voxels,k(A.volume_mm3),A.min,A.max,A.q1,A.median,A.q3,k(A.mean),k(A.stdev)].join(","));return K.join(`
`)+`
`}function ra(P){const r=new Blob([da(P)],{type:"text/csv"}),k=URL.createObjectURL(r),q=document.createElement("a");q.href=k,q.download="mask_stats.csv",document.body.appendChild(q),q.click(),q.remove(),URL.revokeObjectURL(k)}const Sa=P=>String(P).replace(/[&<>"]/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[r]),Ma=P=>{const r=P/1e3;return r>=10?Math.round(r).toLocaleString():r.toFixed(1)};function ba(P,r){const k=Math.max(...P.map(A=>A.volume_mm3)),q=A=>n&&n.R&&n.R[A.label]!=null?`rgb(${n.R[A.label]},${n.G[A.label]},${n.B[A.label]})`:"#6b9bd1";let K="";for(const A of P){const na=r>0?A.volume_mm3/r*100:0,Q=k>0?A.volume_mm3/k*100:0,j=`<span class="stat-val" data-cm3="${Ma(A.volume_mm3)}" data-pct="${na.toFixed(1)}%">${Ma(A.volume_mm3)}</span>`,W=`<span class="stat-bar" style="width:${Q.toFixed(2)}%;background:${q(A)}" data-w-abs="${Q.toFixed(2)}" data-w-pct="${na.toFixed(2)}"></span>`,Y=(ca,ya)=>`<div><span class="k">${ca}</span><span class="v">${ya}</span></div>`,la=Y("min",A.min)+Y("max",A.max)+Y("Q1",A.q1)+Y("Q3",A.q3)+Y("median",A.median)+Y("mean",A.mean.toFixed(2))+Y("SD",A.stdev.toFixed(2))+Y("voxels",A.voxels.toLocaleString());K+=`
        <div class="stat-row" role="button" tabindex="0" data-label="${A.label}" style="cursor:pointer">
          <div class="stat-line">
            <span class="stat-name">${Sa(A.name)}</span>
            <span class="stat-track">${W}</span>
            ${j}
      ...
      <div id="statsPanel">
        <div class="stat-head">
          <span class="stat-total" id="statsUnitLabel">total ${Ma(r)} cm³</span>
        </div>
